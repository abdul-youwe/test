...
class SlideshowComponent extends SliderComponent {
    constructor () {
        ...
        if (this.slider.dataset.draggable === 'true') {
    ...
    initDragBehavior () {
        ...
        this.slider.addEventListener('mouseup', this.endDragging.bind(this));
        this.slider.addEventListener('mouseleave', this.endDragging.bind(this));
        this.slider.addEventListener('dragstart', (e) => e.preventDefault());

        // touch support just incase
        ...
        this.slider.addEventListener('touchend', (e) => this.endDragging(e.changedTouches[0]));
    }
    ...
    startDragging (event) {
        ...
        this.startScrollPos = this.slider.scrollLeft;
    ...
    drag (event) {
        ...
        requestAnimationFrame(() => {
            this.slider.scrollLeft = this.startScrollPos + diff;
        });
    }
    ...
    endDragging (event) {
        ...
        const diff = this.startPos - event.clientX;

        this.slider.style.scrollBehavior = 'smooth';

        if (Math.abs(diff) > this.dragThreshold) {
            const targetPage = diff > 0 ? this.currentPage + 1 : this.currentPage - 1;
            const validTarget = targetPage > 0 && targetPage <= this.sliderItemsToShow.length;

            if (validTarget) {
                this.slideScrollPosition = (targetPage - 1) * this.sliderItemOffset;
            } else {
                this.slideScrollPosition = (this.currentPage - 1) * this.sliderItemOffset;
            }
        } else {
            this.slideScrollPosition = (this.currentPage - 1) * this.sliderItemOffset;
        }
        ...
        requestAnimationFrame(() => {
            this.slider.scrollLeft = this.slideScrollPosition;
        });
    ...
    setSlidePosition (position) {
        ...
        requestAnimationFrame(() => {
            this.slider.scrollTo({
                left: position,
                behavior: this.slider.style.scrollBehavior,
            });
        });
    }
